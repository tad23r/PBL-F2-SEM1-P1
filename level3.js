let starAbilityCooldown = 0;
const STAR_COOLDOWN_TIME = 600; // 10 seconds at 60fps
...
let circleAbilityActive = false;
let circleAbilityCooldown = false;

function activateCircleAbility() {
    if (player.shape !== 'circle' || circleAbilityCooldown) return;

    circleAbilityActive = true;
    circleAbilityCooldown = true;
    console.log("Circle ability activated! Walk through phase walls.");

    setTimeout(() => {
        circleAbilityActive = false;
        console.log("Circle ability ended.");
    }, 7000); // 37 seconds active

    setTimeout(() => {
        circleAbilityCooldown = false;
        console.log("Circle ability ready again.");
    }, 10000); // 10 seconds cooldown
}


function activateStarAbility() {
    // Only works for star shape
    if (player.shape !== "star") return;
    canUseStarAbility = true;

    // Cooldown check
    if (!canUseStarAbility) return;

    const teleportDistance = 200;
    const proposedX = player.x + teleportDistance;

    const collision = levels[currentLevel].platforms.some(platform => {
        return (
            proposedX < platform.x + platform.width &&
            proposedX + player.width > platform.x &&
    ...
    if (!collision) {
        player.x = proposedX;
        canUseStarAbility = false;

        // Cooldown timer (10 seconds = 10000 ms)
        setTimeout(() => {
            canUseStarAbility = true;
        }, 10000);
    } else {
        console.log("Teleport blocked by platform!");
    }
}
...
function drawStarCooldown() {
    ...
    ctx.textAlign = "left";
    ctx.fillText(
        starAbilityCooldown === 0
            ? "Star Smash Ready! (F)"
            : `Cooldown: ${(starAbilityCooldown / 60).toFixed(1)}s`,
        20,
        40
    );
}
...
const levels = [

    {
        ...
        platforms: [
            ...
            { x: 200, y: 160, width: 500,  height: 20 },
            { x: 700, y: 160, width: 20,  height: 160 },
            { x: -70, y: -200, width: 120, height: 2000 },
            { x: 900, y: -200, width: 120, height: 2000 },
        ],
        spikes: [
            { x: 290, y: 290, width: 400, height: 10 },

        ],
        door: { x: 620, y: 230, width: 40, height: 60, type: "pentagon" },
        orbs: [
            { x: 170, y: 460, radius:10, collected:false, offset:0 },
            { x: 250, y: 240, radius:10, collected:false, offset:0 }
        ],
        message: "Level 3: Shape Combos"
    },
    
    {
        ...
        platforms: [
            ...
            { x: 520, y: 420, width: 440, height: 20 },
            { x: 520, y: 0, width: 20, height: 420, type: "death" },
            { x: -70, y: -200, width: 120, height: 2000 },
            { x: 900, y: -200, width: 120, height: 2000 },
        ],
        spikes: [
            { x: 0, y: 490, width: 300, height: 10 }
        ],
        door: { x: 760, y: 360, width: 40, height: 60, type: "star" },
        orbs: [

            { x: 350, y: 450, radius:10, collected:false, offset:0 }
        ],
        message: "Walk spikes (Pentagon) → mid‑air Teleport (Star)"
    },
...

    {
        spawn: { x: 50, y: 460 },
        platforms: [
            { x: 200, y:-200, width: 200, height: 2000, type:"phase" },
            { x: 550, y:-200, width: 60, height: 2000, type:"death" },
            { x: 0,   y: 500, width: 960, height: 40 },
            { x: -70, y: -200, width: 120, height: 2000 },
            { x: 900, y: -200, width: 120, height: 2000 },
        ],
        door: { x: 860, y: 440, width: 40, height: 60, type: "star" },
        orbs: [
            { x: 120, y: 460, radius:10, collected:false, offset:0 },
            { x: 450, y: 460, radius:10, collected:false, offset:0 }
        ],
    },
    {
        spawn: { x: 400, y: 460 },
...
function Platforms() {
    ...
    platforms.forEach(p => {
        ...
        // Choose fill color based on platform type
        if (p.type === "death") {
            ctx.fillStyle = "#8B0000"; // Dark red for death platforms
        } else if (p.type === "phase") {
            ctx.fillStyle = "#1A2B7B";
            if (circleAbilityActive) {
                ctx.globalAlpha = 0.4; // Lighter blue for phase-through platforms
            }
        } else {
            ctx.fillStyle = "#010230"; // Default platform color
        }
...
function checkSpikeDeath() {
    if (player.shape === "pentagon") return; // Immune to spikes when pentagon
...
function handleCollisions() {
    ...
    platforms.forEach(p => {
        // Skip collision with phase walls if ability is active
        if (p.type === 'phase' && circleAbilityActive) return;
    ...
    platforms.forEach(p => {
        ...
        if (
            ...
            if (p.type === "death") {
                loadLevel(currentLevel); // restart the level
                return; // skip further collision processing
            }
...
function checkJump() {
    ...
    const tryingToJump = player.jumpBuffer > 0;
    const onGroundOrCoyote = player.grounded || player.coyoteTimer > 0;
    ...
    if (tryingToJump) {
        if (onGroundOrCoyote) {
            ...
            player.jumpBuffer = 0;
        } else if (isTriangle && !player.hasDoubleJumped) {
            player.jumpHoldTimer = player.maxJumpHold + 5; // Boosted jump
    ...
    if (player.jumpHoldTimer > 0) {
        ...
        const boostMultiplier = player.hasDoubleJumped && !player.grounded ? 1.3 : 1; // Boost if it's the second jump
        const jumpStrength = (player.minJumpPower + (player.jumpPower - player.minJumpPower) * holdFactor) * boostMultiplier;
